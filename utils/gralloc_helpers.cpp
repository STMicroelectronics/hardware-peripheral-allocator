...
uint32_t gralloc_convert_format(uint32_t format)
{
        ...
        case HAL_PIXEL_FORMAT_YV12:
                // fix for Cts GraphicsMapperStableCTests, GetPixelFormatFourCC
        ...
        const uint32_t drm_format = drv_convert_gralloc_format_to_drm_format(format);
        if (drm_format != 0)
                return drm_format;
...
uint64_t gralloc_convert_usage(uint64_t usage)
{
        ...
        handle_usage(&usage, GRALLOC_USAGE_PRIVATE_0, &use_flags, BO_USE_GPU_TILED_VIV);
        handle_usage(&usage, GRALLOC_USAGE_PRIVATE_1, &use_flags, BO_USE_GPU_TS_VIV);
#if (ANDROID_SDK_VERSION >= 29)
        handle_usage(&usage, GRALLOC_USAGE_PRIVATE_3, &use_flags, BO_USE_GPU_NO_CLEAR_VIV);
#else
        handle_usage(&usage, GRALLOC_USAGE_PRIVATE_2, &use_flags, BO_USE_GPU_NO_CLEAR_VIV);
#endif
        if (usage) {
...
{
        gralloc_handle_p hnd = gralloc_handle_p(handle);
        if (!hnd || hnd->magic != GRALLOC_PRIVATE_HANDLE_MAGIC)
                return nullptr;
...
uint32_t drv_resolve_format(uint32_t format, uint64_t use_flags __unused) {
    return format;
}
